...
type Placeable = Pick<Code, 'id' | 'mnemonic'> & { labelIds: string[] };
 ...
 *
 * Matching is case-insensitive substring on the mnemonic (the slug — the code's
 * sole identifier); a fuzzy matcher would surface confident nonsense on a
 * two-letter mnemonic. An EMPTY query returns everything (the picker opens as a
 * browsable list, not a blank slate).
 *
 * Ranking, most-useful first:
 *   1. mnemonic prefix   — you typed "AMB" and meant the code called AMB
 *   2. any other substring hit
...
export function searchCodes(
  ...
  for (const c of codes) {
    ...
    else if (mnemonic.includes(q)) rank = 2;
